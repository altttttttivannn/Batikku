    ...
    <div class="bg-white p-8 rounded-lg shadow-lg w-full max-w-lg">
        ...
        <form method="POST" action="/admin/barang/{{ $barang->id }}" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="nama" class="w-full border rounded px-3 py-2" value="{{ $barang->nama }}" required>
            </div>
            <div class="mb-4">
                <label class="block font-semibold mb-1">Deskripsi</label>
                <textarea name="deskripsi" class="w-full border rounded px-3 py-2">{{ $barang->deskripsi }}</textarea>
            </div>            <div class="mb-4">
                ...
                <div class="grid grid-cols-4 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" name="stok[{{ $size }}]" 
                               ...
                               value="{{ $barang->stokUkuran->firstWhere('ukuran', $size)->stok ?? 0 }}"
                               min="0" required>
                    </div>
                    @endforeach
                </div>
            ...
            <div class="mb-4">
                ...
                <input type="number" name="harga" class="w-full border rounded px-3 py-2" value="{{ $barang->harga }}" required>
            </div>
            <div class="mb-4">
                <label class="block font-semibold mb-1">Kategori</label>
                <select name="kategori" class="w-full border rounded px-3 py-2" required>
                    <option value="kemeja" {{ $barang->kategori == 'kemeja' ? 'selected' : '' }}>Kemeja</option>
                    <option value="celana" {{ $barang->kategori == 'celana' ? 'selected' : '' }}>Celana</option>
                    <option value="accesories" {{ $barang->kategori == 'accesories' ? 'selected' : '' }}>Accesories</option>
                </select>
            </div>
            <div class="mb-6">
                ...
                @if($barang->gambar)
                    <img src="/storage/{{ $barang->gambar }}" alt="{{ $barang->nama }}" class="w-20 h-20 object-cover mt-2 rounded-lg">
                @endif
            </div>
